...
trnd.psi <- 0.9854334 # 0.9525030, 0.9854334, 0.9963604, or 1 when trnd = 0.9, 0.95, 0.98, or 1, respectively
...
n.trns <- 150 #Select from 60, 90, 120, 150
n.pnts <- 10 #No. surveyed points per transect (Set to 5 or 10)
...
path <- paste(path,"/Tr",n.trns,"Pt10Mf1",sep="")
if(!dir.exists(path)) dir.create(path)
...
model_cp <- "F:/research stuff/FS_PostDoc/Occupancy_analysis_simulations/WHWO_R6_monitoring/Power_analysis_via_simulation/scripts/Model_jags_constant_p.R"
...
if(any((MaxN-frst_n)<0)) {
  ...
  ind <- sample.int(length(ind),size=count,prob=frst_area[ind]/sum(frst_area[ind]))
  for(j in ind) frst_n[j] <- frst_n[j] + 1
  }
#________________________________________________#

n.vsts <- matrix(n.vst, nrow = n.trns, ncol = n.yrs) # Required with newer version of constant-p model to allow for partial repeat-visit scenarios
for(i in 1:sims) {
  ...
  for(f in 1:length(forest)) {
    ...
    ID <- dat$V2
    dat <- substr(dat[,V3],3,(n.vsts*20+2))
    ...
    for(tr in 1:dim(y)[1]) for(pt in 1:dim(y)[2]) {
      ...
      y[tr,pt,,] <- matrix(obs,nrow=n.vsts,ncol=20)
    }
  ...
  inits <- function()
    list (Z=z.init)
  parameters <- c("p","PSI")
  out<-jags(data,inits,parameters,model_cp,n.thin=nt,n.chains=nc,n.burnin=nb,n.iter=ni)
...
PSIprd_cp.sim <- PSIprd_yp.sim <- pprd_yp.sim <- array(NA,dim=c(sims,n.yrs,3))
pprd_cp.sim <- matrix(NA,sims,3)
PSIprd_cp.trend <- PSIprd_yp.trend <- matrix(NA,sims,5)
...
for(i in 1:sims) {
  ...
  Rhat.max[i] <- max(c(mod_cp$BUGSoutput$summary[,"Rhat"],mod_yp$BUGSoutput$summary[,"Rhat"]))
  neff.min[i] <- min(c(mod_cp$BUGSoutput$summary[,"n.eff"],mod_yp$BUGSoutput$summary[,"n.eff"]))
  
  # Store occupancy estimates from constant-p model
  ...
  assign(paste0("PSI_cp",str_pad(i, width = 3, pad = "0")), PSI)
  
  # Store detectability estimates from constant-p model
...
# Trend estimates for constant-p model
PSI <- mget(paste0("PSI_cp", str_pad(1:sims, width = 3, pad = "0")))
rm(list = paste0("PSI_cp", str_pad(1:sims, width = 3, pad = "0")))
PSIprd.tnd <- lapply(PSI, function(x) apply(logit(x), 1, trend))

PSIprd_cp.trend[,1] <- sapply(PSIprd.tnd, median)
PSIprd_cp.trend[,2] <- sapply(PSIprd.tnd, function(x) quantile(x,prob=0.025,type=8))
PSIprd_cp.trend[,3] <- sapply(PSIprd.tnd, function(x) quantile(x,prob=0.975,type=8))
PSIprd_cp.trend[,4] <- sapply(PSIprd.tnd, function(x) mean((exp(x) - trnd)^2))
PSIprd_cp.trend[,5] <- sapply(PSIprd.tnd, function(x) mean((exp(x) - trnd.psi)^2))
...
dimnames(PSIprd_cp.trend)[[2]] <- dimnames(PSIprd_yp.trend)[[2]] <- c("median","lo95","hi95","MSE","MSE.psi")

rm(PSIprd.tnd,ID,Y.arry,dat,data,f,i,ind,frst_n,frst_area,frst,mod_cp,mod_yp,
