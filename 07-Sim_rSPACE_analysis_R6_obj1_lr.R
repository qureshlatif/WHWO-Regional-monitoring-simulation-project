...
trnd.psi <- 0.9963604 # 0.9525030, 0.9854334, 0.9963604, or 1 when trnd = 0.9, 0.95, 0.98, or 1, respectively
...
n.trns <- 60 #Select from 60, 90, 120, 150
...
PSIprd_lr.trend <- matrix(NA,sims,5)
...
for(i in 1:sims) {
  ...
  PSIprd_lr.trend[i,5] <- mean((exp(PSIprd.tnd) - trnd.psi)^2)
}
dimnames(PSIprd_lr.trend)[[2]] <- c("median","lo95","hi95","MSE","MSE.psi")
...
save.image(paste(path,"Results_lr.RData",sep=""))
####################################################################################################################
